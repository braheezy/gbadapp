...
def create_sequential_embedded():
    """Convert sequential compressed tiles into Zig code"""

    # Get compressed tile files
    compressed_dir = "assets/sequential/tiles/compressed"
    tile_files = glob.glob(f"{compressed_dir}/tile_*.lz77")
    tile_files.sort()

    print(f"Found {len(tile_files)} compressed tile files")
    ...
    for tile_file in tile_files:
        ...
        tile_data.append(compressed_data)
        print(f"  {os.path.basename(tile_file)}: {len(compressed_data)} bytes")

    # Generate Zig code
    zig_content = f"""// Auto-generated compressed tile data using GBA LZ77
// This file contains the LZ77 compressed tile data

pub const TILE_COUNT = {len(tile_files)};
pub const TILE_SIZE = 16;
pub const TILE_BYTES = TILE_SIZE * TILE_SIZE;

// Compressed tile data array - each tile is compressed with LZ77
pub const compressed_tile_data: [TILE_COUNT]struct {{ data: [*]const u8, len: usize }} = .{{
"""
    for i, data in enumerate(tile_data):
        hex_data = ", ".join([f"0x{b:02x}" for b in data])
        zig_content += f"    .{{ .data = &[_]u8{{ {hex_data} }}, .len = {len(data)} }}, // tile_{i:04d}\n"
...
"""
    ...
    output_path = "assets/sequential/sequential_tiles.zig"
    with open(output_path, 'w') as f:
        f.write(zig_content)

    print(f"Generated {output_path}")
    print(f"Total compressed size: {sum(len(d) for d in tile_data)} bytes")

if __name__ == "__main__":
    create_sequential_embedded()
